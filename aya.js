...
if (game_data.locale == "ar_AE") {
    ...
    langShinko = [
        "الاغارات",
        "اختيار الوحدات للغاره",
        "اختيار نوع الاغارة المستخدمه ",
        "ما المدة الزمنيه المراد ارسال الاغارات بها",
...
(function () {
  // ===== إعدادات (عدّل النص حسب زرّك) =====
  const FIRST_BUTTON_TEXT = "حساب المدة لكل صفحة"; // نص الزر في النافذة الأولى
  const CLICK_ALL_BUTTONS_IN_SECOND_POPUP = true;  // يضغط كل الأزرار في النافذة الثانية
  ...
  function findButtonsByText(text) {
    const t = norm(text);
    const candidates = getVisibleElements("button, input[type='button'], input[type='submit'], a");
  ...
  // ===== منطق التنفيذ =====
  async function run() {
    // 1) انتظر الزر الأول ثم اضغطه
    const firstBtn = await waitFor(() => findButtonsByText(FIRST_BUTTON_TEXT)[0]);
    console.log("[AutoClick] Clicking first button:", FIRST_BUTTON_TEXT);
    clickElement(firstBtn);

    if (!CLICK_ALL_BUTTONS_IN_SECOND_POPUP) return;

    // 2) بعد الضغط، انتظر ظهور مجموعة أزرار جديدة (النافذة الثانية)
    // هنا ننتظر زيادة واضحة في عدد الأزرار الظاهرة (تقريبًا)
    await new Promise(r => setTimeout(r, 400));

    // 3) اضغط كل الأزرار الظاهرة داخل آخر نافذة منبثقة
    // نحاول أخذ "أعلى" عنصر overlay/dialog ظاهر ثم نضغط أزراره
    const popups = getVisibleElements("div, table, section");
    // نختار الأكثر "فوق" عبر أعلى zIndex
    const popup = popups
      .map(el => ({ el, z: parseInt(getComputedStyle(el).zIndex || "0", 10) || 0 }))
      .sort((a, b) => b.z - a.z)[0]?.el;

    // لو ما قدرنا نحدد popup، نضغط كل الأزرار الظاهرة في الصفحة كحل عام
    ...
    // فلترة إضافية: إذا الأزرار في النافذة الثانية كلها نفس الشكل/النص،
    // ممكن تخليها بدون فلترة وتضغط الكل
    console.log("[AutoClick] Found buttons to click:", buttons.length);

    // اضغطها كلها (بهدوء حتى لا يصير تعليق)
  ...
  run().catch(err => console.warn("[AutoClick] Stopped:", err.message));
})();
