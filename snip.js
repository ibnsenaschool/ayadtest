...
function fnWriteCookie(ele){
	...
	window.$("#combined_table tr:first th img[src*=unit_]").each(function(i,e){
		snipeConfig+=window.$("#view_"+e.src.match(/unit\_(.+)\.webp?/i)[1]).is(':checked')?"1":"0";
	});
...
function fnInjectUnits(){
	...
	window.$("#combined_table tr:first th img[src*=unit_]").each(function(i,e){
		...
		{
			window.$('<input type="checkbox" style="cursor: pointer" '+((!twCookie||(twCookie[i]=="1"))?'checked="true"':'')+' id="view_'+e.src.match(/unit\_(.+)\.webp?/i)[1]+'" OnClick="fnWriteCookie(this);"/>').insertBefore(window.$(this.parentNode));
		}
		else
		{
			window.$('<input type="checkbox" style="cursor: pointer" '+((!twCookie||(twCookie[i]=="1"))?'checked="true"':'')+' id="view_'+e.src.match(/unit\_(.+)\.webp?/i)[1]+'" OnClick="fnWriteCookie(this);"/>').insertBefore(window.$(this));
		}
	});
...
function fnExtractUnits(){
	...
	window.$("#combined_table tr:first th img[src*=unit_]").each(function(i,e){
		units.push(e.src.match(/unit\_(.+)\.webp?/i)[1]);
	});
