/*
 * Auto Market Sell Bot v5.4
 */
(function(){
    var _0x1=["https://YOUR_USERNAME.github.io/YOUR_REPO/subscribers.json","ms_saved_user"];
    var SUBS_URL=_0x1[0],STORAGE_KEY=_0x1[1];
    var CLICK_DELAY=1000,BETWEEN_SELL_DELAY=10000,MERCHANT_CAPACITY=1000,BUFFER=500,RESOURCE_BUFFER=1000,DEFAULT_INTERVAL=30000,DEFAULT_THRESHOLD=101;
    
    var TEXTS={title:"بيع تلقائي",enterUsername:"أدخل اسم المستخدم",checking:"جاري التحقق",userNotFound:"اسم المستخدم غير موجود",expired:"انتهى اشتراكك",renewSub:"يرجى تجديد الاشتراك",welcome:"مرحباً",wood:"خشب",stone:"طمي",iron:"حديد",autoSell:"بيع تلقائي",sound:"صوت",stop:"إيقاف",start:"تشغيل",runNow:"تشغيل الآن",close:"إغلاق",period:"الفترة",threshold:"العتبة",log:"السجل",day:"يوم",days:"أيام",reserve:"احتياطي",running:"البوت يعمل",noMerchants:"لا يوجد تجار",done:"تم",checkNow:"فحص",sold:"تم البيع",thresholdMet:"العتبة تحققت",nextCheck:"الفحص القادم",seconds:"ثانية",plan:"الباقة",expiryDate:"تاريخ الانتهاء",connectionError:"خطأ في الاتصال",tryAgain:"حاول مرة أخرى",remaining:"متبقي",savedUser:"مستخدم محفوظ",login:"دخول",changeUser:"تغيير المستخدم",rememberMe:"تذكرني",logout:"تسجيل خروج"};
    
    function getSavedUser(){try{return localStorage.getItem(STORAGE_KEY)||"";}catch(e){return"";}}
    function saveUser(u){try{localStorage.setItem(STORAGE_KEY,u);}catch(e){}}
    ...
    function showLoginDialog(callback){
        ...
        var html='<h2 style="margin:0 0 20px;color:#ffc107;">💰 '+TEXTS.title+'</h2>';
        ...
        html+='<input type="text" id="ms-username" style="width:100%;padding:12px;border:none;border-radius:8px;font-size:16px;text-align:center;margin-bottom:10px;box-sizing:border-box;" placeholder="'+TEXTS.enterUsername+'">'+
        ...
        function doLogin(username){
            ...
            callback(username.toLowerCase(),overlay,rememberCb?rememberCb.checked:true);
        }
        
        btn.onclick=function(){doLogin(input.value.trim());};
        input.onkeypress=function(e){if(e.key==="Enter")doLogin(input.value.trim());};
        cancelBtn.onclick=function(){overlay.remove();};
        if(useSavedBtn)useSavedBtn.onclick=function(){doLogin(savedUser);};
    }
    ...
    function showExpiredDialog(userName,expiryDate,contact){
        ...
        dialog.innerHTML='<h2 style="margin:0 0 20px;font-size:22px;">⛔ '+TEXTS.expired+'</h2><p style="font-size:16px;margin-bottom:8px;">'+TEXTS.welcome+' <strong>'+userName+'</strong></p><p style="margin-bottom:20px;font-size:14px;">'+TEXTS.expiryDate+': <strong>'+expiryDate+'</strong></p><p style="font-size:18px;color:#ffeb3b;margin-bottom:20px;">'+TEXTS.renewSub+'</p><div style="background:rgba(255,255,255,0.2);padding:12px;border-radius:8px;"><p style="margin:0;">📱 '+contact+'</p></div><button onclick="this.parentElement.parentElement.remove()" style="margin-top:20px;padding:10px 30px;background:white;color:#cc0000;border:none;border-radius:8px;font-weight:bold;cursor:pointer;">'+TEXTS.close+'</button>';
        overlay.appendChild(dialog);
        document.body.appendChild(overlay);
    }
    ...
    function showError(msg,overlay){
        ...
        if(btn){btn.textContent=TEXTS.tryAgain;btn.disabled=false;}
        if(useSavedBtn)useSavedBtn.disabled=false;
    }
    
    function checkSubscription(username,overlay,remember){
        fetch(SUBS_URL+"?t="+Date.now()).then(function(r){if(!r.ok)throw new Error("HTTP "+r.status);return r.json();}).then(function(data){
            var user=data.users[username];
            if(!user){showError(TEXTS.userNotFound,overlay);return;}
            var now=new Date(),expiry=new Date(user.expiry);
            fetch("https://worldtimeapi.org/api/ip").then(function(r){return r.json();}).then(function(t){now=new Date(t.datetime);checkExpiry(now,expiry,user,data.settings,overlay,username,remember);}).catch(function(){checkExpiry(now,expiry,user,data.settings,overlay,username,remember);});
        }).catch(function(err){showError(TEXTS.connectionError+": "+err.message,overlay);});
    }
    
    function checkExpiry(now,expiry,user,settings,overlay,username,remember){
        if(now>expiry){overlay.remove();showExpiredDialog(user.name,user.expiry,settings.contact);return;}
        if(remember)saveUser(username);
    ...
    function startBot(user,daysLeft,settings,username){
        ...
        var enabled=true,autoSell=true,sound=true,interval=DEFAULT_INTERVAL,threshold=DEFAULT_THRESHOLD,timerWorker=null,countdownTimer=null,countdown=interval/1000,logBox=null;
        ...
        async function autoSellCheck(){
            ...
            for(var i=0;i<list.length;i++){var r=list[i];var val=getMarketVal(r);if(val===null){log(r+" val not found");continue;}
