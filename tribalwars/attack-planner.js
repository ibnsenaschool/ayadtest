...
(async function () {
    'use strict';
    async function getTargetVillageIdMap(coordsSet) {
    const map = new Map();

    const res = await fetch(window.location.origin + '/map/village.txt');
    ...
    txt.split('\n').forEach(line => {
        ...
        const id = parts[0];      // ID القرية
        const name = parts[1];    // اسم القرية (009)
        const x = parts[2];       // X
        const y = parts[3];       // Y
...
// ===== استخراج ID قرى اللاعب (قراءة فقط) =====
function getMyVillageIdMap() {
    const map = new Map();
    document.querySelectorAll('#combined_table tbody tr').forEach(row => {
        const coord = row.textContent.match(/\b\d{3}\|\d{3}\b/)?.[0];
        const link = row.querySelector('a[href*="village="]');
        const id = link?.href.match(/village=(\d+)/)?.[1];
        if (coord && id) map.set(coord, id);
    });
    return map;
}

// ===== استخراج ID قرى الهدف من السيرفر =====
...
container.appendChild(extractCoordsBtn);
 ...
 planBtn.addEventListener('click', async () => {
     const myVillageIds = getMyVillageIdMap();

    const targets = targetsInput.value.split(/\s+/).filter(Boolean);
     const targetSet = new Set(targets);
const targetVillageIds = await getTargetVillageIdMap(targetSet);
        ...
        targets.forEach(toCoord => {
            ...
            for (let i = 0; i < candidates.length && used < count; i++) {
                ...
                const noteFormatted = note
                    ...
                    : '';
const fromId = myVillageIds.get(fromCoord) || '?';
const toId = targetVillageIds.get(toCoord) || '?';

resultLines.push({
    launch: launchTime,
   text: `${launchStr} | [color=${noteColor}]${unit}[/color] | من (ID:${fromId}) ${fromCoord} → إلى (ID:${toId}) ${toCoord}${noteFormatted}`
